...
class TestGithubOrgClient(unittest.TestCase):
    ...
    def test_public_repos(self, get_json_mock):
        """test public_repos"""
        Dennis = {"name": "Dennis", "license": {"key": "a"}}
        Wanjeri = {"name": "Wanjeri", "license": {"key": "b"}}
        Denno = {"name": "Denno"}
        get_json_mock.return_value = [Dennis, Wanjeri, Denno]
        with patch('client.GithubOrgClient._public_repos_url', PropertyMock(return_value="www.yes.com")) as y:
            x = GithubOrgClient("x")
            self.assertEqual(x.public_repos(), ['Dennis', 'Wanjeri', 'Denno'])
            self.assertEqual(x.public_repos("a"), ['Dennis'])
...
class TestIntegrationGithubOrgClient(unittest.TestCase):
    """testing repos method using integration"""
